        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                try:
                    ...
                    """, (one_week_ago,))
                    active_users_weekly = (await cursor.fetchone())[0]
                    ...
                    await cursor.execute("""
                        ...
                        WHERE last_activity >= %s
                    """, (one_month_ago,))
                    active_users_monthly = (await cursor.fetchone())[0]

                    # Новые пользователи за последнюю неделю
                    await cursor.execute("SELECT COUNT(*) FROM users WHERE registration_date >= %s", (one_week_ago,))
                    new_users_weekly = (await cursor.fetchone())[0]
